...
export default function AuctionBox({ status, title, price, timeLeft, imageUrl }) {
    const statusStyles = {
      ...
      upcoming: { bg: 'bg-purple-100', text: 'text-purple-800', badge: 'Coming Soon' },
    ...
    return (
      <div className="flex-1 bg-white rounded-xl shadow-md overflow-hidden hover:shadow-lg transition-shadow mx-2">
        ...
        <div className="h-48 relative">
          ...
          <span className={`absolute top-2 right-2 px-3 py-1 rounded-full text-sm font-medium ${statusStyles[status].bg} ${statusStyles[status].text}`}>
        ...
        <div className="p-4">
          ...
          <div className="flex justify-between items-center mb-3">
            ...
            <div className="text-right">
              ...
              <p className={`font-medium ${status === 'ongoing' ? 'text-red-600' : 'text-gray-700'}`}>
                {timeLeft}
              </p>
            </div>
          </div>
          ...
          <button className={`w-full py-2 rounded-lg font-medium transition-colors ${
            status === 'sold' ? 'bg-gray-300 text-gray-700 cursor-not-allowed' 
            : status === 'ongoing' ? 'bg-blue-600 hover:bg-blue-700 text-white' 
            : 'bg-purple-600 hover:bg-purple-700 text-white'
          }`}>
            {status === 'sold' ? 'View Details' : status === 'ongoing' ? 'Place Bid' : 'Notify Me'}
          </button>
        </div>
      </div>
    );
