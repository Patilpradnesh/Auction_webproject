...
app.post("/addbids", async (req, res) => {
    try {
        const { item, yourBid, leading, endTime } = req.body;

        // Validate input fields
        if (!item || !yourBid || isNaN(yourBid)) {
            return res.status(400).json({ status: "error", message: "Valid item and bid amount are required" });
        }

        // If endTime is not provided, set it to 24 hours from now
        const bidEndTime = endTime ? new Date(endTime) : new Date(Date.now() + 24 * 60 * 60 * 1000);
        ...
        const newBid = new Bid({ item, yourBid, leading, endTime: bidEndTime });
        await newBid.save();
